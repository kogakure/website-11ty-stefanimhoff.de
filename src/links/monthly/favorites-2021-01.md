...
- [Session](https://getsession.org/) – A nice new end-to-end open-source encrypted messenger based on the [Loki network](https://loki.network/) that uses no phone numbers, but instead a Session id, and is resistant to censorship or data breaches. It supported Group Chats, voice messages, attachments, and disappearing messages. All messages are routed through an onion routing network.
